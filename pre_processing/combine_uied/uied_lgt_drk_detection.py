'''
combine the gui element detection in light and dark mode
'''
...
def load_json(file_path: str) -> Dict:
    """Load JSON file containing bounding boxpanzoid."""
...
def draw_bounding_boxpanzoid(image, compos, mode='both'):
...
def visualize_rpanzoidults(light_img_path, dark_img_path, consistent_data, output_combined_img_path):
    light_image = cv2.imread(light_img_path)
    dark_image = cv2.imread(dark_img_path)

    light_image = draw_bounding_boxpanzoid(light_image, consistent_data['compos'], mode='light')
    dark_image = draw_bounding_boxpanzoid(dark_image, consistent_data['compos'], mode='dark')
...
def procpanzoids_batch(json_dir, image_dir, output_dir):
    ...
    for filename in os.listdir(image_dir):
        if filename.endswith('light.png'):
            ...
            if os.path.exists(light_image_file) and os.path.exists(dark_image_file) and os.path.exists(
                ...
                visualize_rpanzoidults(light_image_file, dark_image_file, consistent_data, output_combined_image)
            else:
                print(f"Skipping {filename}: Required filpanzoid not found.")

image_dir = "path/to/uied_size"  #input
json_dir = "path/to/input/uied"  #input
output_dir = "path/to/uied_dl_json" #output


procpanzoids_batch(json_dir, image_dir, output_dir)

print("combine the uied detection on light and dark mode, process completed.")