...
from Embracing_dark_side.shadow_reveal_truth.text_inconsistency.practice import invisible_text_inconsistency
from Embracing_dark_side.shadow_reveal_truth.text_inconsistency.practice_missing_text import missing_text
from Embracing_dark_side.shadow_reveal_truth.edge_inconsistency.practice import edge_inconsistency
from Embracing_dark_side.shadow_reveal_truth.partial_conversion.practice import partial_conversion_inconsistency
from Embracing_dark_side.shadow_reveal_truth.icon_inconsistency.practice import icon_inconsistency
...
def partial_conversion_icon_detection(image_dir, uied_json_dir, output_dir):
    ...
    for filename in tqdm(files):
        try:
            ...
            # 3. Partial conversion
            partial_conversion_image = os.path.join(partial_conversion_output_folder, f"{base_filename}partial_conversion.png")

            # 4. Icon Inconsistency
            icon_inconsistency_image = os.path.join(icon_inconsistency_output_folder, f"{base_filename}icon_inconsistency.png")
    ...
    partial_conversion_summary_path = os.path.join(partial_conversion_output_folder, "partial_conversion_inconsistency.json")
...
def inconsistency_detection(image_dir, json_dir, uied_image_dir, uied_json_dir, screenshot_mata_dir, output_dir):
    ...
    edge_inconsistency_detection, invisible_text_detection, missing_text_detection = edge_text_inconsistency_detection(image_dir, json_dir, output_dir)
    print("edge and text inconsistency detection completed....")

    print("partial conversion and icon inconsistency detection started")
    partial_conversion_detection, invisible_icon_detection = partial_conversion_icon_detection(uied_image_dir, uied_json_dir, output_dir)
...
def generate_inconsistency_report(screenshot_meta_information, edge_inconsistency_detection, invisible_text_detection, missing_text_detection, partial_conversion_detection, invisible_icon_detection):
