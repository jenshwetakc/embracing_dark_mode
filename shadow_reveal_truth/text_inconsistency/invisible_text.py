...
def compare_light_dark_mode_pixels(light_image, dark_image, text, threshold=15):
    ...
    light_image: Light mode image (BGR format).
    dark_image: Dark mode image (BGR format).
    bbox: Bounding box (x1, y1, x2, y2).
    threshold: Allowed pixel difference to classify as identical.

    Returns: "Embedded Text in Image" if pixels are nearly identical, else "Normal Text".
    """
...
def rgb_to_hex(rgb):
    ...
    Ensures all values are integers before formatting.
    """
